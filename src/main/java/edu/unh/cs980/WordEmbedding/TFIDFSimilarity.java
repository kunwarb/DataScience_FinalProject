...
public class TFIDFSimilarity {
	 ...
	 * @Description: Main function of the Similarity which calculates similarity score of paragraph
	...
	public void writeTFIDFScoresTo(String runfile) throws IOException, ParseException {
		...
		for (Data.Page page : pageList) { // For every page in .cbor.outline We
											// need...
			...
			for (TermQuery query : terms) { // For every Term
				...
				for (int i = 0; i < tpd.scoreDocs.length; i++) { // For every
					...
					if(!docResults.contains(dResults.getParagraphId()))
					{
			...
			for (Map.Entry<Document, Float> entry : scores.entrySet()) { // For
																			// every
																			// document
																			// and
																			// its
			...
			while ((current = docQueue.poll()) != null) {
				...
				if(!docResults.contains(current))
